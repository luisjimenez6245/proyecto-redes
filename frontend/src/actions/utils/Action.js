...
import { API_URL } from "configuration/manager";
...
class Action {
  ...
  paramsToQuery = (filters) => {
    ...
    for (let filter in filters)
      if (filters[filter] != null) query += `${filter}=${filters[filter]}&`;
    return query;
  };
  ...
  request = (method, path, query, body, callback, toDisp, otherArgs = {}) => {
  ...
  requestExternal = (method, callback, otherArgs = {}) => {
    let args = {};
    args = {
      method: method,
      ...otherArgs,
